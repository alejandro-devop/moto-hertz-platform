...
**Una sección sin lista delante reusa las mismas anclas de ficha, a mano.** `paginas.ficha` y `configuracion.ficha` (Fase 4) no tienen `tourDeLista` que llamar —ninguna de las dos abre desde una fila— y `tourDeFicha` tampoco encajaba del todo, porque sus textos («Ficha de una moto») asumen un registro entre muchos y acá cada uno es el único que existe. Se escriben a mano en `registry.ts`, pero **reusando** `ficha.secciones` / `ficha.obligatorio` / `ficha.guardar` en vez de anclas propias: nunca hay dos fichas abiertas a la vez en el panel, así que reusarlas fuera de un `FormSheet` es tan seguro como dentro de uno.

> **Y por eso `SiteSettingsForm` se registra como `ControlDeFicha` por su cuenta**, con las mismas tres líneas que ya tenía `form-sheet.tsx` (sin `open` del que depender: esta ficha nunca se desmonta mientras la página existe). El campo obligatorio de Configuración (`siteName`) vive en la pestaña «Textos», la última, no en «Contacto», la que abre por defecto — el caso exacto que `seccion` existe para resolver, pero `seccion` depende de ese puente. Sin el registro, el paso se saltaba siempre en silencio: su ancla nunca estaba en la pestaña ya abierta. `PaginasPage` no lo necesita: no tiene pestañas, no hay a dónde saltar.

**El panel «Ayuda y recorridos» (`ayuda-y-recorridos.tsx`) lista los `TOURS_ORDENADOS`** con su estado — visto / saltado / sin ver, comparando `version` guardada contra `TOURS[clave].version`, la misma cuenta que hace el provider para decidir si relanza — y un reinicio por fila (`tours.reiniciar(clave)`, ya existía desde la Fase 0). Sin botón en «sin ver»: no hay nada que reiniciar. El reinicio individual no lleva diálogo de confirmación, a diferencia del global: uno solo es exactamente lo que el usuario pidió al tocarlo, con el mismo costo que el «?» de una sección; borrarlo todo de una vez sí lo justifica.

**El teclado y el foco no hubo que resolverlos: ya los resuelve `driver.js`, y sin querer los resuelve bien.** `disableActiveInteraction: true` le pone al elemento señalado `pointer-events: none !important` (clase `driver-no-interaction`) — y la propia función que arma la lista de elementos «tabulables» de la librería descarta cualquier elemento con `pointer-events: none`. Consecuencia no buscada pero verificada: el elemento señalado **nunca entra al ciclo de `Tab`**, así que el foco nunca se escapa del popover ni cae sobre un botón real (un "Publicar moto", un `<Select>`) que dispararía algo mientras el recorrido sigue abierto. `Esc` cierra y marca `skipped` (mismo camino que el botón «×»); el popover ya trae `role="dialog"` y `aria-labelledby`/`aria-describedby` de fábrica. Verificado con `Input.dispatchKeyEvent` sobre CDP: seis `Tab` seguidos sin salir del popover, y un `Esc` que cierra y deja la fila en la base con `status: skipped`.

**Verificado a 390×844 real (no solo con el navegador angosto de escritorio).** El filtrado `solo: 'movil' | 'escritorio'` deja los recorridos correctos por pantalla (bienvenida en 3 pasos en vez de 5, con los de `panel.tabbar`), ningún popover midió fuera del viewport en ninguna sección probada —`driver.js` reubica solo cuando no cabe—, y la ficha a pantalla completa con un paso `seccion` (horarios de un punto) abre la pestaña y señala el editor igual que en escritorio. `prefers-reduced-motion: reduce` sí quita la clase `driver-fade` del `<body>` (`animate: !prefiereMenosMovimiento()` en `run-tour.ts`, ya estaba desde la Fase 0).
