...
**Alcance mínimo acordado con el usuario (Fase 5), ampliado después por un pedido de MEJORAS.md.** El documento de la fase dejaba abierto cuánto de la home administrar; se eligió el mínimo: solo el **carrusel de banners** era administrable desde el panel, y el segundo banner ancho («Financia tu próxima Yamaha») quedaba fijo en el código de `web`. Eso cambió con la migración `013` (`slot`, catálogo cerrado `HOME` | `SECUNDARIO` — mismo criterio que `service_points.type`, migración `007`, no texto libre: el valor decide en qué componente de `web` se pinta, así que lo controla el dominio): ahora `home_banners` administra los dos banners del home, cada uno con su propio orden (`position` es por slot, no global — `nextPosition`/`reorderBanners` reciben el slot). Los títulos de cada sección de la home (`Servicios Yamaha`, `Últimas Noticias`, sus subtítulos) siguen **fijos en el código de `web`**, eso no cambió. La home tampoco inventa sus propias tarjetas de servicios/noticias: `web/src/app/page.tsx` consulta las mismas `getServices()`/`getNews()` que ya usan `/servicios` y `/noticias` — antes vivían hardcodeadas y duplicadas en `web/src/data/home-mock.json`, borrado en esta fase junto con toda la capa de emulación de Contentful que solo servía a la home (`services/contentful.ts`, `hooks/useHomeData.ts`, `components/contentful/`, `types/contentful.ts`, `utils/contentful-resolver.ts`, `app/api/contentful/home/`).
...
- **`position` es el dato que ordena el carrusel de cada slot**, y no se acepta en `bannerAdd`/`bannerEdit`: un banner nuevo se agrega al final de su propio slot (`nextPosition(slot)` cuenta los no borrados de ese slot) y **solo cambia con `bannerReorder(slot: BannerSlot!, ids: [ID!]!)`**, que reescribe la posición de todos los banners no borrados de ese slot como el índice de su id en el arreglo que llega, dentro de una transacción — no toca los demás slots.
- **`slot` es obligatorio al crear, opcional al editar** (se puede mover un banner de slot): catálogo cerrado `HOME` | `SECUNDARIO`, no texto libre — ver la nota de la migración `013`.
...
**Textos era un solo campo, `siteName`.** Deliberado en la Fase 6 — el usuario decidió que solo el nombre del sitio se administraba (repetido antes en al menos ocho sitios: `footer-data.json`, `layout.tsx`, `manifest.json`, componentes de la home, autor por defecto de una noticia sin firma); la descripción de la empresa del footer y el resto de textos institucionales quedan fijos en el código, fuera de alcance. **`logo` se sumó después** (migración `014`, pedido en `docs/cms-plan/MEJORAS.md`): mismo criterio de imagen opcional que `seoImage` (`urlSchema.optional()`, no acepta `null` explícito — se reemplaza, no se limpia desde la ficha). `NULL` = `web/src/components/menu/Menu.tsx` sigue usando el asset estático (`/assets/logos/yamaha.svg`).
